(() => {
  ...
  const spotlightSection = el("spotlight-section");
  const spotlightCard = el("spotlight-card");
...
function renderDetailAction(anime, context) {
  ...
  // Report an issue only makes sense for posts in the local library —
  // News/Spotlight items are pure discovery, nothing to report there.
  ...
  // News tab items (Trending/Popular) are pure discovery, not tied to a
  // local post — no Join / Request action.
...
// ---------------------------------------------------------------------
// Spotlight card (top of News tab — real anime news, not a catalog item)
// ---------------------------------------------------------------------
function renderSpotlight() {
  ...
  if (spotlight.image) {
    ...
    img.alt = spotlight.title;
    card.appendChild(img);
  }
  const badge = document.createElement("span");
  badge.className = "spotlight-badge";
  badge.textContent = "#1 SPOTLIGHT";
  card.appendChild(badge);

  const textWrap = document.createElement("div");
  textWrap.className = "spotlight-overlay-text";
  const title = document.createElement("p");
  ...
  card.addEventListener("click", openSpotlightDetail);
  spotlightCard.appendChild(card);
}

function openSpotlightDetail() {
  if (!spotlight) return;
  openDetailSheet({
    title: spotlight.title,
    description: spotlight.summary || "No summary available.",
    genres: [],
    poster_url: spotlight.image,
    link: spotlight.link,
  }, "spotlight");
}
...
el("report-submit").addEventListener("click", async () => {
  ...
  async function loadSpotlight() {
    try {
      spotlight = await api("/api/news/spotlight");
    } catch (err) {
      spotlight = null;
    }
    renderSpotlight();
  ...
  (async function init() {
    ...
    await Promise.all([loadNews(), loadAvailable(), preloadProfile(), loadSpotlight()]);
    renderAvailableTab();
  })();
})();  
